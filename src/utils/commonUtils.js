...
commonUtils.variableValueExists = function (value, returnDoesNotExist) {
	...
	if (returnDoesNotExist !== "") {
		returnDoesNotExist = returnDoesNotExist || ""
	}
	if (!isFinite(value)) {
...
/**
 * 字符串截取
 * @deprecated 使用了弃用的符号，请使用commonUtils.strSlice方法
 ...
 * @param fillLength {Number}补零的位数如小于10补为:01(最高支持到千即:0001) 默认为两位数
 * @returns {string|Number} 处理完成和返回的值
 */
commonUtils.fillWith0 = function (value, fillLength) {
 ...
 * @returns {String|Infinity|NaN|Number} 拼接好的字符串
 */
...
export {dateUtils, urlUtils, storageUtils, countDownUtils, commonUtils, domUtils}
